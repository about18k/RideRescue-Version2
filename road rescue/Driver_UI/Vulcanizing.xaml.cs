...
{
    public partial class Vulcanizing : ContentPage
    {
        ...
        public Vulcanizing(Location location, Supabase.Client supabase)
        {
            try
            {
                InitializeComponent();
                ...
                LoadVulcanizingShops();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error initializing Vulcanizing: {ex}");
                DisplayAlert("Error", "Failed to load vulcanizing shops", "OK");
            }
        }

        private async void LoadVulcanizingShops()
        {
            try
            {
                Places.Clear();

                // 1. Fetch all vulcanizing shops
                var placesResponse = await _supabase
                    .From<PlaceModel>()
                    .Select("*")
                    .Where(x => x.category == "vulcanizing")
                    .Get();
